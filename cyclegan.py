...
class CycleGAN():
    ...
    def sample_image(self, epoch, batch_i):
        os.makedirs('images/%s/oe' % self.dataset_name, exist_ok=True)
        r, c = 2, 3
        ...
        # Demo (for GIF)
        imgs_A = self.data_loader.load_img('test.jpeg')
        imgs_B = self.data_loader.load_img('test.jpeg')
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("images/%s/oe/%d_%d.png" % (self.dataset_name, epoch, batch_i))
        plt.close()
